...
TRANSLATION_MODEL = "gpt-4o-mini" # Or allow user selection

# Processing Parameters
CHUNK_DURATION = 120 # seconds, adjust based on VRAM

# Subtitle Styling (Can be made configurable in UI later)
SUBTITLE_FONT = 'C:/Windows/Fonts/Verdana.ttf'
SUBTITLE_FONTSIZE = 24
SUBTITLE_COLOR = 'white'
SUBTITLE_BG_COLOR = 'rgba(0, 0, 0, 0.5)' # Black with 50% opacity
...
CHUNK_SRT_DIR = os.path.join(OUTPUT_DIR, "chunk_srts") # Store intermediate SRTs here
...
def srt_time_to_seconds(time_str):
    ...
    try:
        ...
        # Raise the error to be caught by the main processing loop
...
def safe_remove(path):
    ...
    try:
        ...
        print(f"Error removing {path}: {e}") # Log error, maybe report to user later
...
def extract_full_audio(video_path, output_dir, progress_queue=None):
    ...
    # Use a simpler name, maybe without timestamp unless needed for uniqueness in parallel runs
    ...
    try:
        ...
        # Use ffmpeg specific codec for wider compatibility if needed, pcm_s16le is standard WAV
        video.audio.write_audiofile(audio_path, codec='pcm_s16le', logger=None) # Suppress verbose moviepy logs
        video.close()
        if progress_queue: progress_queue.put("Audio extracted successfully.")
        return audio_path
    except Exception as e:
        safe_remove(audio_path) # Clean up partial file
...
def split_audio_into_chunks(audio_path, chunk_duration, output_dir, progress_queue=None):
    ...
    try:
        ...
        while start_time < duration:
            ...
            # Avoid zero-duration clips
            if end_time <= start_time:
                break

            chunk_filename = f"audio_chunk_{chunk_number}.wav" # Simpler names
            chunk_path = os.path.join(output_dir, chunk_filename)
            # if progress_queue: progress_queue.put(f"Creating chunk {chunk_number}: {start_time:.1f}s - {end_time:.1f}s") # Maybe too verbose
        ...
        print(f"Error during audio splitting ({audio_path}): {e}") # Log details
        # Attempt cleanup
        for chunk_path, _, _ in chunks:
            safe_remove(chunk_path)
        if audio: audio.close()
        # Report error and re-raise
...
def transcribe_with_whisper(model, audio_path, progress_queue=None):
    ...
    try:
        # Determine fp16 based on device availability
        use_fp16 = model.device.type == 'cuda'
        # Consider adding more options like beam_size, language detection etc. if needed
        result = model.transcribe(audio_path, fp16=use_fp16)
        # if progress_queue: progress_queue.put(f"Transcription complete for: {os.path.basename(audio_path)}") # Optional success message
        return result
    except Exception as e:
        # Specific error for OOM
...
def translate_subtitles(client, subtitles, target_language, custom_prompt_template, token_usage, progress_queue=None):
    ...
    # Filter out empty texts before creating the batch prompt
    ...
    if not indexed_texts:
        ...
        return subtitles # Return original if nothing to translate

    # Create batch prompt using original indices
    ...
    # --- Use Custom Prompt ---
    # Replace placeholder in the custom prompt provided by the user
    system_prompt = custom_prompt_template.replace("{target_language}", target_language)
    # Add instructions about maintaining numbering if not already in the user's prompt
    ...
    # --- End Custom Prompt Usage ---

    try:
        response = client.chat.completions.create(
            model=TRANSLATION_MODEL, # Use the globally defined model for now
            messages=[
        ...
        # Create the final list, preserving original structure
        translated_subtitles_list = subtitles[:] # Create a copy
        for original_index, _ in indexed_texts:
            # Get the translation using the 1-based index used in the prompt
            translation = parsed_translations.get(original_index + 1)
            if translation:
                translated_subtitles_list[original_index]['text'] = translation
            else:
                # If translation for a specific line failed or was missing, keep original? Or mark as error?
                 print(f"Warning: Missing translation for subtitle number {original_index + 1}. Keeping original.")
                 # Optionally add a note: translated_subtitles_list[original_index]['text'] += " [Translation Failed]"

        if progress_queue: progress_queue.put("Translation finished.")
        return translated_subtitles_list

    except Exception as e:
        # Fallback to individual translation is slow and costly - maybe just fail here?
        print(f"Batch translation error: {e}")
        if progress_queue: progress_queue.put(f"❌ Batch translation failed: {e}. Skipping translation for this chunk.")
        # Return original subtitles for this chunk if batch fails
        return subtitles
...
def read_srt(srt_path):
    ...
    try:
        ...
        # Handle different line endings and split into blocks
        ...
        for entry_str in entries:
            ...
            if len(lines) >= 3:
                ...
                if number.isdigit() and '-->' in timing and text:
                    # Normalize text (replace internal newlines with spaces)
                    text = text.replace('\n', ' ')
                    subtitles.append({
                        'number': number,
                        'timing': timing,
...
def process_audio_chunk(whisper_model, openai_client, chunk_path, chunk_number, total_chunks, start_time_offset, target_language, custom_prompt_template, token_usage, output_dir, progress_queue=None):
    """Process single chunk: transcribe, translate, generate SRTs, report progress."""
    ...
    try:
        ...
        if not transcription or 'segments' not in transcription or not transcription['segments']:
            ...
            # Write empty files to prevent downstream errors
            write_srt([], original_srt_path)
            write_srt([], translated_srt_path)
            return original_srt_path, translated_srt_path # Return paths even if empty

        # 2. Create original SRT data (relative to chunk start)
        ...
        for i, segment in enumerate(transcription['segments']):
             ...
             if text: # Only add segments with actual text
                ...
                original_subtitles_data.append({
                     'number': str(i+1),
                     # Times are relative to the chunk start, handled during combination
        ...
        # 3. Translation
        # Read back the just-written SRT to ensure format consistency
        original_subtitles_parsed = read_srt(original_srt_path)
        if not original_subtitles_parsed:
            if progress_queue: progress_queue.put(f"Warning: Original SRT for chunk {chunk_number} is empty after writing/reading. Skipping translation.")
            write_srt([], translated_srt_path) # Write empty translated SRT
        ...
        translated_subtitles = translate_subtitles(
            ...
            token_usage,
            progress_queue # Pass queue down
        )
        ...
        # Log error and ensure placeholder files exist if needed
        ...
        # Ensure empty files exist so combine step doesn't fail completely
        ...
        # Decide if one chunk failure should stop everything (re-raise) or continue (return paths)
        # For now, let's continue but the combined file might be incomplete
        return original_srt_path, translated_srt_path
        # OR: raise e # Uncomment to stop processing on first chunk error
...
def combine_srt_files(srt_paths, output_path, chunk_start_times, progress_queue=None):
    ...
    for i, (srt_path, chunk_start_offset) in enumerate(zip(srt_paths, chunk_start_times)):
        if not srt_path or not os.path.exists(srt_path) or os.path.getsize(srt_path) == 0:
            # print(f"Skipping combination for empty/missing SRT (index {i}): {srt_path}")
...
def embed_subtitles(video_path, srt_path, output_path, subtitle_style, progress_queue=None):
    ...
    try:
        ...
        # Define subtitle generator function using provided style
        def subtitle_generator(txt):
            # Apply styling from the passed dictionary
            return TextClip(txt,
                            ...
                            size=(video_width * 0.9, None), # Allow 90% width max
                            align='center') # Alignment within the caption box

        subtitles = SubtitlesClip(srt_path, subtitle_generator)

        # Ensure subtitles don't exceed video duration (handles potential SRT timing errors)
        # Set start is often unnecessary if SRT timings are absolute and correct
        subtitles = subtitles.set_duration(video_duration)
        # Position using the style dictionary
        subtitles = subtitles.set_position(subtitle_style.get('position', ('center', 'bottom')))

        final_clip = CompositeVideoClip([video, subtitles])

        # Write video file with specific parameters for compatibility/quality
        final_clip.write_videofile(output_path,
                                   codec='libx264',       # Good compatibility
                                   audio_codec='aac',     # Standard audio codec
                                   temp_audiofile=temp_audio_path, # Explicit temp audio path
                                   remove_temp=True,      # Remove temp audio file
                                   preset='medium',       # Balance speed/quality (ultrafast, fast, medium, slow, slower)
                                   ffmpeg_params=['-crf', '23'], # Constant Rate Factor (lower=better quality, larger file, 18-28 is reasonable)
                                   threads=os.cpu_count() or 4, # Use available cores
                                   logger=None) # Suppress FFMPEG command output
        ...
        if "Could not find font" in str(e):
            err_msg = f"❌ Error: Font '{subtitle_style.get('font')}' not found by MoviePy/ImageMagick. Try 'Arial', 'Verdana' or check installation."
        elif "Invalid duration" in str(e) or "Non-positive duration" in str(e):
             err_msg = f"❌ Error: Problem with video/subtitle duration matching. Check SRT timings, especially near the end."
        elif "OSError: MoviePy Error: creation of None failed" in str(e) and "check that you installed Imagemagick" in str(e):
             err_msg = "❌ Error: ImageMagick dependency likely missing or misconfigured. Please install ImageMagick and ensure MoviePy can find it."

        if progress_queue: progress_queue.put(err_msg) # Report user-friendly error
        return False # Embedding failed
    finally:
        # Ensure resources are closed
        if video: video.close()
        if final_clip: final_clip.close()
        safe_remove(temp_audio_path) # Clean up temp audio just in case
...
def process_video_threaded(params, progress_queue, result_queue):
    ...
    Uses helper functions and reports progress via queue.
    """
    ...
    whisper_model = None
    openai_client = None
    # Define directories based on constants
    ...
    try:
        ...
        output_options = params["output_options"] # 'SRT', 'Embed', 'Both'
        ...
        os.makedirs(OUTPUT_DIR, exist_ok=True) # Ensure final output dir exists

        # Initialize OpenAI client
        progress_queue.put("Initializing OpenAI client...")
        try:
             openai_client = OpenAI(api_key=api_key)
             # Optional: Add a simple test call like listing models to verify key early
        ...
        # 3. Process Chunks (Transcribe & Translate)
        original_srt_paths = []
        translated_srt_paths = []
        progress_queue.put(f"Starting chunk processing (total {total_chunks})...")
        for i, (chunk_path, start_time, _) in enumerate(chunks_info):
            # Pass all necessary parameters down, including the custom prompt
            orig_srt, trans_srt = process_audio_chunk(
                 whisper_model, openai_client, chunk_path, i+1, total_chunks, start_time,
            ...
            # Optional: Add cleanup for the individual chunk .wav file here if memory is tight
            # safe_remove(chunk_path)
        ...
        # 5. Embed Subtitles (if requested)
        final_video_path = None
        ...
        if output_options in ['Embed', 'Both']:
            ...
            # Define subtitle style dictionary from constants
            subtitle_style = {
                'font': SUBTITLE_FONT,
            ...
            embedding_successful = embed_subtitles(
                video_path,
                combined_translated_srt, # Use the combined translated SRT
                final_video_path,
                subtitle_style,
                progress_queue
            )
            if not embedding_successful:
                # Embedding failed or was skipped, ensure final_video_path is None
                final_video_path = None
        else:
        ...
        # Send a user-friendly error message to the UI
        result_queue.put(f"❌ Error: {e}")
    finally:
        # --- Cleanup ---
        progress_queue.put("Cleaning up temporary files...")
        # Remove the main extracted audio file
        safe_remove(full_audio_path)
        # Remove the directory containing audio chunks
        safe_remove(temp_audio_chunks_dir)
        # Optionally remove intermediate chunk SRTs unless useful for debugging
        # safe_remove(output_chunk_srts_dir)
        progress_queue.put("🧹 Cleanup attempted.")

        # Make sure the queue knows processing is finished, regardless of success/failure
        end_time_total = time.time()
        progress_queue.put(f"Processing finished in {end_time_total - start_time_total:.2f} seconds.")


# --- Streamlit UI---

st.set_page_config(layout="wide", page_title="Video Subtitle Translator")
st.title("🎬 Video Subtitle Translator")
st.markdown("Upload a video, get transcribed & translated subtitles (powered by Whisper & OpenAI).")
...
if not st.session_state.processing: # Only cleanup if not actively processing
    safe_remove(TEMP_DIR)
    # Optionally clear output dir too, or keep results from previous runs
...
with col1:
    ...
    # Handle file persistence across reruns
    if uploaded_file:
         if not st.session_state.processing:
             st.session_state.current_video_file = {
                 "name": uploaded_file.name,
                 "data": uploaded_file.getvalue() # Read data into memory ONLY when new/changed
             }
    ...
    output_options = st.radio(
        "Output Options",
        ('Generate Translated SRT file', 'Generate Both'),
        index=1,
        horizontal=True,
        disabled=st.session_state.processing,
...
with col2:
    ...
    custom_prompt_template = st.text_area(
        ...
        DEFAULT_TRANSLATION_PROMPT, # Use the default directly now
        height=150,
...
if not st.session_state.processing:
    ...
    start_button = False # Ensure start logic isn't re-triggered
...
# Display persistent status/progress if processing
...
if start_button:
    ...
    # Save uploaded file to temp dir (using stored data)
    ...
    output_choice = 'SRT'
    if output_options == 'Embed Subtitles into New Video':
        output_choice = 'Embed'
    elif output_options == 'Generate Both':
        output_choice = 'Both'
    ...
    processing_params = {
        ...
        "api_key": api_key,
        "custom_translation_prompt": custom_prompt_template, # Pass the template directly
        "output_options": output_choice,
    }
    ...
    while st.session_state.worker_thread.is_alive():
        ...
        while not progress_queue.empty():
            ...
            if "Extracting audio" in message: progress_value = max(progress_value, 0.05)
                 ...
                 try:
                    ...
                    if len(parts) == 2:
                       progress_value = max(progress_value, 0.20 + (int(parts[0]) / int(parts[1])) * 0.50) # Chunks are 50%
                 except: pass
        ...
        time.sleep(0.3) # Check queue less aggressively

    # Thread finished
    while not progress_queue.empty(): # Process any final messages
        message = progress_queue.get()
        st.session_state.progress_message = message
        if "finished" in message: # Ensure progress reaches 100
    ...
    result = result_queue.get() # Get final result or error
    ...
    st.session_state.processing = False
    st.session_state.worker_thread = None
    # Keep current_video_file so user sees what was processed if they don't upload new one
    st.rerun() # Rerun to show results/clear inputs
